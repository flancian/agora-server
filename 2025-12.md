...
*   Basic `Inbox` and `Outbox` routes exist but likely need completion.

### Planned Work
*   [ ] **Follower Management:**
    *   Implement `Inbox` handling for `Follow` activities.
    *   Store followers in SQLite (`followers` table).
    *   Implement `Accept` activity response.
*   [ ] **Content Broadcasting (Outbox):**
    *   Hook into the "new subnode" event (indexer/worker).
    *   Construct `Create` activities for new notes.
    *   Push activities to followers' inboxes.
*   [ ] **Star Federation:**
    *   When a user stars a node in Agora, publish a `Like` or `Announce` activity.
    *   Allow users to "Follow" the stars of others?
