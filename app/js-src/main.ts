...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  */
  ...
  async function loadAsyncContent() {
    ...
    if (!synthesisButton || !synthesisContent || !synthesisDetails || !synthesisContainer) {
    ...
    const runSynthesis = async (provider: string) => {
        ...
        try {
            const response = await fetch(`/api/synthesize/${nodeUri}?provider=${provider}`);
            const data = await response.json();
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', (e) => {
            ...
            // Always trigger synthesis on tab click
            const provider = tab.getAttribute('data-provider') || 'mistral';
            runSynthesis(provider);
        });
    });

    synthesisButton.addEventListener('click', (e) => {
