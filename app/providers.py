...
def cache_ai_generation(provider_func):
    ...
    It now handles storing and retrieving both the full prompt and the content.
    """
    @functools.wraps(provider_func)
    def wrapper(prompt):
        provider_name = provider_func.__name__.split('_')[0]
        ...
        cached_full_prompt, cached_content, timestamp = sqlite_engine.get_ai_generation(prompt, provider_name)
        ...
        if cached_content and (time.time() - timestamp) < ttl:
            ...
            return cached_full_prompt, cached_content
        
        current_app.logger.info(f"AI Cache: MISS for '{prompt[:30]}...' with provider {provider_name}.")
        full_prompt, new_content = provider_func(prompt)
        ...
        if "error" not in new_content.lower() and "not properly set up" not in new_content.lower():
            sqlite_engine.save_ai_generation(prompt, provider_name, full_prompt, new_content, int(time.time()))
        
        return full_prompt, new_content
    return wrapper
...
def gemini_complete(prompt):
    if current_app.config["ENABLE_AI"]:
        ...
        if not api_key:
            error_message = "[[GenAI]] is not properly set up for Gemini in this Agora yet. Please set the GEMINI_API_KEY environment variable to a valid API key."
        ...
        try:
            ...
            return enriched_prompt, response.text
        except Exception as e:
            return enriched_prompt, f"An error occurred with the Gemini API: {e}"
    else:
...
def mistral_complete(prompt):
    if current_app.config["ENABLE_AI"]:
        ...
        if not api_key:
            error_message = "[[GenAI]] is not properly set up for Mistral in this Agora yet. Please set the MISTRAL_API_KEY environment variable to a valid API key."
            return None, error_message
        ...
        try:
            ...
            return enriched_prompt, chat_response.choices[0].message.content
        except Exception as e:
            return enriched_prompt, f"An error occurred with the Mistral API: {e}"
    else:
        return None, "<em>This Agora is not AI-enabled yet</em>."
